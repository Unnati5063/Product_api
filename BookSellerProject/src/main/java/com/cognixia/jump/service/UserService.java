...
public class UserService {
	...
	public String userCheckOut(Integer user_id) {
		...
		for (Map.Entry<Integer, Integer> b : counter.entrySet()) {
			...
			if (b.getValue() > book.get().getrating()) {
				bookRepo.updateBookInventory(book.get().getId(), 0);
				counter.put(book.get().getId(), counter.get(book.get().getId()) - book.get().getrating());
				continue;
			}
			bookRepo.updateBookInventory(book.get().getId(), book.get().getrating() - b.getValue());
